...
import { rabinKarpSearch } from './rabin-karp';

describe('Algoritma Rabin-Karp (RK))', () => {

    it('menemukan satu kecocokan kata dengan benar', () => {
        const text = "Awas ada situs maxwin88 di sini";
        const keywords = ["maxwin88"];
        const result = rabinKarpSearch(keywords, text);
    ...
    it('menangani string kosong dan array keyword kosong dengan aman', () => {
        expect(rabinKarpSearch([], "Beberapa teks")).toEqual([]);
        expect(rabinKarpSearch(["slot"], "")).toEqual([]);
        expect(rabinKarpSearch([], "")).toEqual([]);
    });
